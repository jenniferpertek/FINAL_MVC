...
namespace BLL
{
    public class ExchangeRESTService
    {
        ...
        public List<CurrencyDetailDTO> GetExchanges()
        {
            string uri = baseUri;
            using (HttpClient httpClient = new HttpClient())
            {
                Task<String> response = httpClient.GetStringAsync(uri);
                return JsonConvert.DeserializeObject<List<CurrencyDetailDTO>>(response.Result);
            }
        }

        public bool PostExchanges(CurrencyDetailDTO p)
        {
            string uri = baseUri;
            using (HttpClient httpClient = new HttpClient())
            {
        ...
        public bool PutExchanges(CurrencyDetailDTO p)
        {
            string uri = baseUri;
            using (HttpClient httpClient = new HttpClient())
            {
